...
async fn create_components(
    ...
    // Set AnyTLS buffer sizes from experimental config (or reset to defaults)
    let duplex = config
        .experimental
        .as_ref()
        .and_then(|exp| exp.anytls_duplex_buffer_size)
        .unwrap_or(16 * 1024);
    let relay = config
        .experimental
        .as_ref()
        .and_then(|exp| exp.anytls_relay_buffer_size)
        .unwrap_or(4 * 1024);
